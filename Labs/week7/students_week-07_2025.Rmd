...
```{r}
Kset <- seq(from = 0, to = 1000,length.out = 20) #densities I want to look at
...
for(j in 1:length(Kset)){
  # assign the value of Y
	...
	Rstarset[j] <- R.simuhw[length(tset1)]
	Nstarset[j] <- N.simuhw[length(tset1)]
	Mstarset[j] <- M.simuhw[length(tset1)]
