...
head(S.hw1a) # use head() to look at the first 6 elements
tail(S.hw1a) # use tail() to look at the last 6 elements
...
head(S.hw1b) # use head() to look at the first 6 elements
tail(S.hw1b) # use tail() to look at the last 6 elements
...
head(S.hw1c) # use head() to look at the first 6 elements of 
tail(S.hw1c) # use tail() to look at the last 6 elements of 




Icola <- 'black' # color to use for a
Icolb <- 'dodgerblue3' # color to use for plotting b
Icolc <- 'forestgreen' # color to use for plotting c
...
**As infectiousness increases, R_0 increases linearly and p_crit increases asymptotically. USing intuition this makes sense because as infectiousness increases, more people can become infected which leads to. ahigher R_0. Similarly, higher infectiousness causes a need for a higher p_crit, but once it reaches a certain point p_crit is as close to 100% as possible **

## 5b: Recovery time

```{r}
# create a vector of values of beta over which to calculate R0
gamma_set <- seq(from = 0.001, to = 0.1, length.out = 100) # sequence from 0.001 to 0.1 with a total length of 100

# calculate R0 for each value of beta
R0_set.gamma <- beta*N/gamma_set # R0 = beta*N/gamma

# calculate p_crit for each value of R0
p_crit_set.gamma <- 1 - 1/R0_set.gamma  # p_crit = 1 - 1/R0


# plot of $R_0$ (y-axis) vs. Recovery time (x-axis)
plot(x = gamma_set, y = R0_set.gamma, # x = recovery, y = basic reproductive number for each value of recovery time
     type = 'l', las = 1, lwd = 2, # plot aesthetics
     xlab = 'Recovery Time (gamma)', ylab = 'Basic Reproductive Number (R_0)') # axis labels

# plot of $p$ (y-axis) vs. Recovery time (x-axis)
plot(x = gamma_set, y = p_crit_set.gamma, # x = recovery, y = critical vaccination rate needed to prevent an epidemic for each value of infectiousness
     type = 'l', las = 1, lwd = 2, # plot aesthetics
     xlab = 'Recovery Time (gamma)', ylab = 'Critical Vaccination Rate (p_crit)') # plot labels

```

explain the shapes of the graphs based on the biology of disease spread

**As recovery time increases, R_0 decreases asymptotically and p_crit decreases linearly. This makes sense because R0 is proportional to the infectious period. A longer recovery time means infected individuals remain infectious for a longer duration, but the effect on R0 diminishes as transmission levels out. In contrast, p_crit decreases linearly because it is directly related to R0 through the equation
p_crit = 1−1/R0. **
...
# create a vector of values for N over which to calculate R0
Nset.N <- seq(from = 0, to = 100, length.out = 100) # sequence from 1 to 100 with a total length of 100

# calculate R0 for each value of N
R0_set.N <- beta*Nset.N/gamma # R0 = beta*N/gamma

# calculate p_crit for each value of R0
p_crit_set.N <- 1 - 1/R0_set.N  # p_crit = 1 - 1/R0

# plot R0 as a function of N
plot(x = Nset.N, y = R0_set.N, # x = total population size, y = basic reproductive number for each population size
     type = 'l', las = 1, lwd = 2, # plot aesthetics
     xlab = 'Population size (N)', ylab = 'Basic Reproductive Number (R_0)') # axis labels

# plot of $p$ (y-axis) vs. population size (x-axis)
plot(x = Nset.N, y = p_crit_set.N, # x = infectiousness, y = p crit
     type = 'l', las = 1, lwd = 2, # plot aesthetics
     xlab = 'Population size (N)', ylab = 'Critical Vaccination Rate (p_crit)') # plot labels

```

explain the shapes of the graphs based on the biology of disease spread

**These graphs show that R_0 increases linearly with N and p_crit increases asmyptotically with N. What I find odd about the shape of these graphs is that R_0 and p_crit should remain constant with population size because the equations rely on proportion of sick individuals, not the number of cases.**
