...
As the alternate prey Y increases in density, the stable equilibria of prey X decreases asymptotically and the stable equilibria of predator increases asymptotically. This makes sense because there is more prey available for the predator. However, the prey species X and Y begin competing with each other for reasources.
...
The bifurcation diagrma of X with respect to Y shows apparent competition because they share a common predator. When prey Y has a density of roughly 350-600, we can see that prey species X and Y are predated at a similar rate. However, when Y has lower density X experiences higher predation and vise versa.
...
K_pset <- seq(from = 0, to = 1000,length.out = 50) #densities I want to look at (0-1000 prey items)
# checking to make sure that simulation has reached equilibrium
...
# plot stable equilibria of predator (Pstarset) against density of alternate prey (Yset)
plot(x = K_pset, y = Pstarset,
     type = 'l', lwd = 2, col = Pcol, las = 1,
     xlab = 'Carrying capacity of predator, K_p', ylab = 'Stable equilibria of predator, P*')

# plot stable equilibria of prey (Xstarset) against density of alternate prey (Yset)
plot(x = K_pset, y = Xstarset,
     type = 'l', lwd = 2, col = Xcol, las = 1, 
     xlab = 'Carrying capacity of predator, K_p', ylab = 'Stable equilibria of focal prey, X*')
...
As the carrying capacity of the predator (K_p) increases, it makes intuitive sense for the stable equilibria of the predator to also increase. In other words, when the carrying capacity increases, the equilibria point also increases. Similarly, it makes sense that as the predator carrying capacity increases, the prey stable equilibria decreases. When there are more predators, then more prey are consumed.
...
#	P <- P.simu4a[i-1]
	...
	# calculating change in population size for predator and prey
#	dX <- ( r_x*X*(1-X/K_x) - a_x*P*X - h_x_now*X ) * dt
#	dP <- ( P*(c*(a_x*X+a_y*Y_new)-d_p)*(1-P/K_p) - h_p_now*P ) * dt
	
	# calculating total population size for predator and prey
#	X.simu4a[i] <- X.simu4a[i-1] + dX
#	P.simu4a[i] <- P.simu4a[i-1] + dP
#}
#check head
...
# plot prey (X.simu2) against time (tset)
#plot(x = tset, y = X.simu4a,
#     type = 'l', las = 1, lwd = 2, col = Xcol,
#     xlab = 'Time', ylab = 'Density',
 #    ylim = c(0, 160))

# plot predator (P.simu2) against time (tset)
#lines(x = tset, y = P.simu4a,
 #     lwd = 2, col = Pcol)
...
# start new simulation with values from the end of the previous simulation
#set initial conditions to be the pop valus at equilibrium with hunting aka at the end of the simulation
X0_dis <- X.simu6[length(tset)]
P0_dis <- P.simu6[length(tset)]

head(X0_dis)
#SYNCHRONOUS
# creating holding vectors and storing initial conditions
X.simu7 <- NaN*tset3 #note tset2 and updated to simu3
X.simu7[1] <- X0_dis
P.simu7 <- NaN*tset3
P.simu7[1] <- P0_dis
...
for(i in 2:length(tset3)){
  ...
  ## tstophunt is 120 (time we stop hunting) is the same for both
  ## if the current time is before we stop hunting, then keep on hunting. set the hunting rate to teh hunting rate
	if(tset3[i] < t_stophunt){ h_p_now <- h_p } else { h_p_now <- 0 } ##if after we stop hunting, set to 0, no more hunting!
...
for(i in 2:length(tset2)){
  ...
  # if time is before t_stopX, h_x_now = h_x
  # else, h_x_now = 0
	if(tset3[i] < t_stopnew){ h_x_now <- h_x } else { h_x_now <- 0 }## UPDATE HERE!! important- setting different time to stop hunting
...
for(i in 2:length(tset2)){
  ...
  # if time is before t_stopX, h_x_now = h_x
  # else, h_x_now = 0
	if(tset3[i] < t_stophunt){ h_x_now <- h_x } else { h_x_now <- 0 }## UPDATE HERE!! important- setting different time to stop hunting
...
**The scenarios from fastest to slowest recovery is synchronous, predator first, and then prey first. I think this makes sense because often times predator species are more long lived than their prey. Therefore, it makes sense that it would take the predator population longer to reach equilibrium if they have to wait for their prey to recover last.**


**The overall recovers to an unexploited state was fastest in: ______synchronous_____ scenario**
...
**Our results suggest that the secondary prey type abundance can impact predator first recovery speeds. In our model, we can see that increasing the secondary prey abundance led to the system recovering 100 years later than Samhouri et al (2017) initially predicted.**
