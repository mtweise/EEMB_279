...
## 1. plot population growth as a function of N
################################################
plot(x = Nset, y = growth.log, # Create a line plot with Nset on the x-axis and growth.log on the y-axis
     type = 'l', # Use a line plot ('l' stands for line)
     las = 1, # Rotate axis labels to be horizontal
     lwd = 2, # Set the line width to 2 for better visibility
     xlab = 'N', # Label the x-axis as 'N'
     ylab = 'Population Change') # Label the y-axis as 'Population Change'

# add a line at the x-axis
abline(h = 0, lwd = 1) # Draw a horizontal line at y = 0 to represent equilibrium, with a line width of 1

################################################
# 2. plot the economics of harvest
################################################
lines(x = Nset, y = q*E*Nset, # Add a line plot showing harvest as a function of Nset
      col = 'royalblue2', # Set the line color to royal blue
...
################################################
# unstable
points(x = 0, y = 0, # Plot a point at (0, 0)
       pch = 21,  # Use a point style with a border and optional fill
       cex = 1.5) # Set the size of the point to 1.5

# stable
points(y = Ystar, x = Nstar, # Plot a point at (Nstar, Ystar)
       pch = 21, # Use the same point style as above
       cex = 1.5, # Set the size of the point to 1.5
       bg = 'black') # Fill the point with black to indicate stability

################################################
# 4. add a legend
################################################
legend(x = 0, y = -0.5, # Position the legend at coordinates (0, -0.5)
       legend = c('growth', 'harvest', 'unstable equil.', 'stable equil.'), # Define labels for the legend
       lwd = c(2, 2, NaN, NaN), # Set line widths for applicable legend entries (NaN for no line)
       pch = c(NaN, NaN, 21, 21), # Set point styles for applicable entries (NaN for no point)
       pt.cex = 1.5,  # Set the size of the points in the legend to 1.5
       pt.bg = c('white', 'white', 'white', 'black'), # Set background colors for points in the legend
       col = c('black', 'royalblue2', 'black', 'black'), # Set colors for lines and points in the legend
       ncol = 2)  # Arrange the legend into 2 columns
...
```{r}
N_MSY <- K/2 #N_MSY is the variable K divided by 2
E_MSY <- r/q/2 #E_MSY is the variable r divided by q divided by 2
Y_MSY <- r*K/4 #Y_MSY is the variable r times K divided by 4
...
# 1. biology
###################################
plot(x = Nset, y = growth.over,  # Create a line plot with Nset on the x-axis and growth.over on the y-axis
     type = 'l', # Use a line plot 
     lwd = 2, # Set the line width to 2
     las = 1, # Rotate axis labels to be horizontal
     xlab = 'N', # Label the x-axis as 'N'
     ylab = 'Change in Population') # Label the y-axis as 'Change in Population'
...
# 2. economics
###################################
lines(x = Nset, y = q*E_MSY.over*Nset, # Add a line plot showing harvest as a function of Nset (Y = qEN)
      lwd = 2, # Set the line width to 2
      col = 'royalblue2') # Set the line color to royal blue
...
###################################
# unstable
points(x = 0, y = 0,  # Plot a point at (0, 0), representing an unstable equilibrium
       pch = 21, # Use a point style with a border and optional fill
       cex = 1.5) # Set the size of the point to 1.5

# stable
points(x = N_MSY.over, y = Y_MSY.over, # Plot a point at (N_MSY.over, Y_MSY.over)
       pch = 21,    # Use the same point style as above
       cex = 1.5, # Set the size of the point to 1.5
       bg = 'black') # Fill the point with black to indicate stability
...
# 4. legend
###################################
legend(x = 80, y = Y_MSY.over,  # Position the legend at coordinates (80, Y_MSY.over)
       legend = c('growth', 'harvest', 'unstable equil.', 'stable equil.'), # Define labels for the legend
       lwd = c(2, 2, NaN, NaN), # Set line widths for applicable legend entries (NaN for no line)
       pch = c(NaN, NaN, 21, 21), # Set point styles for applicable entries (NaN for no point)
       pt.cex = 1.5,  # Set the size of the points in the legend to 1.5
       cex = 0.8, # Adjust the text size in the legend to 0.8
       pt.bg = c('white', 'white', 'white', 'black'), # Set background colors for points in the legend
       col = c('black', 'royalblue2', 'black', 'black')) # Set colors for lines and points in the legend
...
```{r}
Y_MSY.over <- max(dNdt.over) #Y_MSY is the max of our dndt.over function
Y_MSY.over #print

#for(i in 1:length(Nset)){ #Loop through each index of the vector Nset
#  if(dNdt.over[i] == Y_MSY.over){ # Check if dNdt.over[i] equals Y_MSY.over
#    N_MSY.over <- Nset[i] # If the condition is true, store the corresponding Nset value as N_MSY.over
#  }
#}
N_MSY.over #print


E_MSY.over <- Y_MSY.over/(q*N_MSY.over) #E_MSY is Y_MSY divided by q times N_MSY
E_MSY.over
...
growth.dep <- dNdt.dep
############################
plot(x = Nset, y = growth.dep, # Create a line plot with Nset on the x-axis and growth.dep on the y-axis
     type = 'l', # Use a line plot 
     lwd = 2, # Set the line width to 2 for better visibility
     las = 1, # Rotate axis labels to be horizontal
     xlab = 'N', # Label the x-axis as 'N'
...
# 2. economics
############################
lines(x = Nset, y = q*E_MSY.dep*Nset, # Add a line plot showing harvest as a function of Nset (Y = qEN)
      lwd = 2, # Set the line width to 2
      col = 'royalblue2')# Set the line color to royal blue
...
############################
# stable
points(x = 0, y = 0,  # Plot a point at (0, 0), representing a stable equilibrium
       pch = 21,  # Use a point style with a border and optional fill
       cex = 1.5, # Set the size of the point to 1.5
       bg = 'black')  # Fill the point with black to indicate stability
points(x = N_MSY.dep, y = Y_MSY.dep, # Plot a point at (N_MSY.dep, Y_MSY.dep)
       pch = 21, # Use the same point style
       cex = 1.5, # Set the size of the point to 1.5
       bg = 'black') # Fill the point with black to indicate stability

# unstable
points(x = N_star_unstable, y = Y.unstable.dep, # Plot a point at (N_star_unstable, Y.unstable.dep)
       pch = 21,  # Use the same point style
       cex = 1.5, # Set the size of the point to 1.5
       bg = 'white') # Fill the point with white to indicate instability

############################
# 4. legend
############################
legend(x = 0, y = -1, # Position the legend at coordinates (0, -1)
       legend = c('growth', 'harvest', 'unstable equil.', 'stable equil.'), # Define labels for the legend
       lwd = c(2, 2, NaN, NaN), # Set line widths for applicable legend entries (NaN for no line)
       pch = c(NaN, NaN, 21, 21), # Set point styles for applicable entries (NaN for no point)
       pt.cex = 1.5, # Set the size of the points in the legend to 1.5
...
```{r}
Y_MSY.dep <- max(dNdt.dep) #Y_MSY.dep is the max of our dndt function
Y_MSY.dep #print
...
#}
N_MSY.dep #print
#this should be 42 (it is in my lab portion, but it doesn't print properly here)

E_MSY.dep <- Y_MSY.dep/(q*N_MSY.dep) #E_MSY.dep is Y_MSY/ (q times N_MSY)
E_MSY.dep #print
```
...
The population experiences a tipping point at the unstable equilibrium point (approximately when N=38). The alternative states it could exhibit is either a population increase towards the stable equilibrium state or a population decrease towards the "trivial" equilibrium state at 0 (aka a population crash).
...
plot(tset, N.simu.dep,  # Plot population size (N) over time (tset)
     type = 'l',  # Use a line plot
     ylim = c(0, K),  # Set y-axis limits between 0 and carrying capacity (K)
     las = 1,  # Make y-axis labels horizontal
     lwd = 2,  # Set line width for better visibility
     xlab = 'Time', ylab = 'Population Size')  # Label x-axis and y-axis

#################################################
# 2. Add lines for different initial conditions
#################################################

lines(tset, N.simu.dep.low,  # Add a line for initial population N0 = 2
      col = 'paleturquoise3',  # Set line color
      lwd = 2)  # Set line width

lines(tset, N.simu.dep.high,  # Add a line for initial population N0 = 33
      col = 'peachpuff3',  # Set line color
      lwd = 2)  # Set line width

lines(tset, N.simu.dep.100,  # Add a line for initial population N0 = 100
      col = 'coral2',  # Set line color
      lwd = 2)  # Set line width
...
legend(x = max(tset)*.6, y = K,  # Position the legend at 60% of the time range
       lwd = 2,  # Match line width to the plot lines
       legend = c('N0=2', 'N0=33', 'N0=34', 'N0=100'),  # Define legend labels for each initial population
       col = c('black', 'paleturquoise3', 'peachpuff3', 'coral2'),  # Match legend colors to plotted lines
       horiz = FALSE)  # Keep the legend items in a vertical layout
...
I would be worried about being at the unstable equilibrium (N=38). I would avoid the problem by extensively modeling and being aware of how to population rate of change is tending towards through time. 
...
growth.sub <- (r*Nset*(1-Nset/K) + M)
growth.sub
harvest.sub <- q*(E_MSY)*Nset
harvest.sub
...
for (i in 1:length(Nset)) {
  if (!is.na(dndt.sub[i]) && dndt.sub[i] == Y_MSY.sub) {
    N_MSY.sub <- Nset[i]
  }
}
...
############################################
# plot the biology in black
plot(x = Nset, y = growth.sub, 
     type = 'l', 
     las = 1, 
     lwd = 2, 
     xlab = 'N', ylab = 'Population Change',
     xlim = c(0, max(Nset) * 1.2),  # Expands the x-axis range
     ylim = c(min(growth.sub) * 1.2, max(growth.sub) * 1.2) )
...
############################################
# plot the economics in blue
lines(x = Nset, y = harvest.sub, col = 'royalblue2', lwd = 2)
...
############################################
# plot the economics in blue
lines(x = Nset, y = harvest.sub1, col = 'royalblue2', lwd = 2)
...
Our E_MSY was suddenly at a smaller population size. Effort increased.
...
E_MSY.sub
```
Stocks experiencing higher fishing effort (E) or higher catchability (q) may have a reduced N_MSY.
If a fishery is over-exploited, its N_MSY may decline compared to an unfished or lightly fished stock. N_MSY was largest for depensating stocks and smallest for subsidized stocks.

E_MSY was highest for the overcompensating stock and lowest for the depensating stock. Overcompensating stocks exhibit density-dependent growth, meaning that as population size decreases, individual survival and reproduction increase significantly. Because of this strong compensatory response, a higher fishing effort is required to reach the point where yield is maximized
